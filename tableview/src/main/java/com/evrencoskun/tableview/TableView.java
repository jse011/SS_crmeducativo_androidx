...
public class TableView extends FrameLayout implements ITableView {
    ...
    public static class Values{
        private float mColumnHeaderHeight;
        private float mRowHeaderWidth;

        public float getmColumnHeaderHeight() {
            return mColumnHeaderHeight;
        }

        public void setmColumnHeaderHeight(float mColumnHeaderHeight) {
            this.mColumnHeaderHeight = mColumnHeaderHeight;
        }

        public float getmRowHeaderWidth() {
            return mRowHeaderWidth;
        }

        public void setmRowHeaderWidth(float mRowHeaderWidth) {
            this.mRowHeaderWidth = mRowHeaderWidth;
        }
    }
    ...
    public TableView(@NonNull Context context, Values values) {
        super(context);
        this.values = values;
        initialDefaultValues(null);
        initialize();
    ...
    private void initialDefaultValues(AttributeSet attrs) {
        ...
        mRowHeaderWidth = (int) (values!=null ? values.getmRowHeaderWidth():  getResources().getDimension(R.dimen.default_row_header_width));
        mColumnHeaderHeight = (int) (values!=null ? values.getmColumnHeaderHeight(): (int) getResources().getDimension(R.dimen.default_column_header_height));
        // Colors
    ...
    protected DividerItemDecoration createItemDecoration(int orientation) {
        ...
        if (mSeparatorColor != -1) {
            // Change its color
            divider.mutate().setColorFilter(mSeparatorColor, PorterDuff.Mode.SRC_ATOP);
        }
