package com.consultoraestrategia.ss_crmeducativo.tablefixheaders;
...
public abstract  class TableFixHeaderAdapter<
    ...
    protected abstract VBODY inflateBody(int row, int column);
    ...
    private View getBody(final int row, final int column, View convertView, ViewGroup parent) {
        //final VBODY vbody = (convertView == null) ? inflateBody(row, column) : (VBODY) convertView;
        if(convertView!=null)((VBODY)convertView).destroyView();
        final VBODY vbody =  inflateBody(row, column);
