  ...
  private HashMap<FileType, List<Document>> createDocumentType(ArrayList<Document> documents) {
    ...
    for (final FileType fileType : fileTypes) {
      ...
      for (Document document : documents){
        Log.d(DocScannerTask.class.getSimpleName(), document.toString());
        Log.d(DocScannerTask.class.getSimpleName(), document.getFileType().toString());
        if(Arrays.equals(document.getFileType().extensions, fileType.extensions)){
          documentListFilteredByType.add(document);
        }
      }
