            ...
            public void onLoadStickerCategory(View view, StickerCategory stickerCategory, boolean selected) {
              try{
                  if (stickerCategory instanceof ShopStickers) {
                      ...
                      if (selected) {
                          DrawableCompat.setTint(DrawableCompat.wrap(dr), AXEmojiManager.getInstance().getEmojiViewTheme().getSelectedColor());
                      }else{
                          DrawableCompat.setTint(DrawableCompat.wrap(dr), AXEmojiManager.getInstance().getEmojiViewTheme().getDefaultColor());
                      }
                      ((AppCompatImageView)view).setImageDrawable(dr);
                  } else {
                      Glide.with(view)
                              .load(Integer.valueOf(stickerCategory.getCategoryData().toString()))
